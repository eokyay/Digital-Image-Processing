...
#define TABLESIZE 512
 ...
 void fillDescription(int r, int g, int b, int index){
   ...
   if (r == 0 && g== 0 && b == 0)
	 ...
	 strcpy(colorDescriptions[index], "OTHER");
 }
...
void subString(char dest[], char src[], int start, int howmany){
    ...
    strncpy(dest, &src[start], howmany);
    dest[howmany] = '\0';
}
  ...
  void analysis_display()
 {
	...
	int i, countColor=0, kk;
	  ...
	  if (occurrenceColor_i > 0)  // this color has been encountered
	  {
	     ...
	     double probCol_Safe = (double)occurrenceColorSafe_i/(double)safePixelCount;  // probability that this color is observed, given that the safe region is observed   
   	     double  prob_Safe =  (double)safePixelCount / (double)allPixelCount; // probability of observing safe region in the image 
	     double probCol =  (double)occurrenceColor_i/(double)allPixelCount; //  probability of observing this color in the image 
	     ...
	     //now retrieve the descriptions of the color (group)
	      if ( ! strcmp(colorDescriptions[i], "OTHER"))  // descriptions is  OTHER
	      {
	         ...
	         kk =0;
	         while (nameSubString[kk] != '\0')
		 {
		     nameSubString[kk] = toupper(nameSubString[kk]);
		     kk ++;
		 }
